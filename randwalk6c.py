'''
Perform many random walks of N steps and histogram the locations and average the distances
'''
...
nStep = 100                      # set number of steps here
nWalk = 100  * nStep             # set number of walks here
nDims = 2                        # set to 1/2/3 dimensions
...
def mainstream():
  ...
  figa.suptitle(str(nWalk) + " Random Walks for " + str(nStep) + " steps in " + str(nDims) + " Dimensions")
  if nDims == 3:
    ax[0].axis('off')
    ax[0] = figa.add_subplot(121,projection='3d')

  xList = []; yList = []; zList = []; dList = [] 
  
  for Seed in range (sSeed, sSeed + nWalk):
    #print("  Seed =", Seed) 
    xLst, yLst, zLst, Dist, Dmax = RWmods.RandomWalk(nDims, Seed, nStep)
    #print("    Posn =", xLst[nStep], yLst[nStep], zLst[nStep], end="")
    #print("    Dist =", Dist, "    Dmax =", Dmax)
    xList.append(xLst[nStep]); yList.append(yLst[nStep]); zList.append(zLst[nStep])
    dList.append(Dist)
    #if Dist > 14:
    #  print("  D =", Dist, "  at:", xLst[nStep], yLst[nStep], zLst[nStep]) 
